...
| `ε`                 | Differential privacy budget **per gossip event** under Laplace mechanism. Cumulative loss over T epochs grows as Tε — a composition identity, not a protocol guarantee; lifetime DP is not claimed (§8.1) | §4.5, §7.4 |
...
Adv_unlink(A, T, k, Π) = | Pr[A(epoch_id_T, epoch_id_{T+k}) = same_sk] − ½ |
                       ≤ ε_PRF(Poseidon) + ε_Π(traffic_correlation, T, k) + ε_cluster(v, T, k, Π)
```
...
**Scope — the behavioral-label channel is carved out, not bounded.** The third term `ε_cluster` is the published behavioral cluster label (§6.2.1) acting as a standing cross-epoch quasi-identifier: a node with a stable fingerprint keeps the same label across `epoch_id` rotations, leaking from public chain data rather than the wire (so `ε_Π` and cover traffic do not touch it). Under the **Tor/I2P** profile the full fingerprint makes this term non-negligible and re-identification is acknowledged out of scope against a global passive adversary. Under **Loopix** the fingerprint is thin (§6.2), so the label is coarse, but `ε_cluster ≤ p_stable · (1 − A_eff/N_w)` is still **not information-theoretically bounded** — `A_eff` (the label's effective anonymity set) collapses to `O(1)–O(10)` in the behavioral tail and in small communities (§6.2.1). It is held down operationally by a `k_floor` publication rule and source-side fingerprint coarsening, with an intrinsic ceiling set by the publicly-recomputable raw fingerprint. This is the same disclosed-channel posture as the P2/PSI carve-out below; quantification is **OQ-65** and the channel should be read into P1 the way the PSI channel is read out of P2.
...
**Scope — P2 covers the gossip channel only.** The game above is defined over `gossip_T(p)`. PSI handshakes (§5.4) reveal intersection information to the counterparty *by design* and are **not** covered by this claim: PSI outputs carry no noise, no permutation, and no DP accounting, and an adversary running repeated PSI against a target accumulates un-noised set-membership information that P2 never bounds. The channel is acknowledged and operationally limited (k_min gate §7.4, PSI cache decay §4.8, `n_psi_in`/`n_discovery` rate limits §5.4), but no information-theoretic bound exists for it — see §8.1 "PSI intersection leakage" and OQ-64.
...
| P2 Preference indistinguishability         | proven (sk) | Pedersen hiding (perfect) + permutation secrecy + **clean `ε`-DP** (clamp post-processing, §4.5) + composition (`ε_eff` open, OQ-55; lifetime `Tε` not claimed). **Gossip channel only — PSI outputs excluded** (§8.1, OQ-64) |
...
- Behavioral fingerprint re-identification rate under Loopix — the `ε_cluster` term in P1. Its anonymity-set structure is now derived (§6.2.1): `A_eff` collapses in the behavioral tail and small communities, label-side mitigations are capped by the publicly-recomputable raw fingerprint, and per-config calibration of `A_eff`/`k_floor` is **OQ-65**. Carved out of P1, not bounded.
...
**Bootstrapping beyond genesis:** The genesis validator set covers the period before enough nodes have accumulated sufficient temporal depth and cluster diversity. When cluster diversity constraints cannot yet be fully satisfied, the behavioral cluster constraint is relaxed during the transition period, falling back to interest cluster diversity only. This behavioral-cluster-enforcement threshold is the cluster-density analogue of the temporal-depth-mass condition in the **normative bootstrap transition criterion (§5.1.1)** — both gate on enough organic nodes having accumulated temporal depth, and both are the empirical residual of OQ-12. §5.1.1 is the single normative home for the bootstrap conditionality; the constants here are calibrated alongside it (OQ-12).
...
**commit_T and n_commit batching:** `commit_T` and its ZK proof are included in the `epoch_transaction` and submitted every epoch. `M_v(T)`, `C_p(T)`, score band, and health tier batch per `n_commit`. The rationale: `commit_T` binds the per-epoch identity (`"VERDICT_FINALIZED epoch_id_T"`) and a fresh per-epoch share split, so batching would create staleness. Under the **adopted publish-`s₁` design (§4.9.4)** it is encrypted to the standing validator key `VA_pub` — no per-epoch committee key is involved. `commit_T` is opaque without a verdict signature and reveals only node presence, which is already inferrable from `epoch_id` registration. *(2-of-2 profile only: `commit_T` is additionally tied to the current epoch's committee threshold BLS key, generated by the VRF-selected committee running a standard DKG over BLS12-381 at epoch start — see §4.9.4.)*

**DKG timing constraint (2-of-2 profile only):** Committee DKG must complete before a node constructs and submits `commit_T`. DKG for K≈21 nodes is fast in practice. The `epoch_transaction` submission deadline is set after DKG completion. Under the adopted publish-`s₁` design no per-epoch committee DKG exists; the analogous liveness obligation is the `VA_pub` PSS re-share window at validator rotation (below; §8.2 T8).
...
> **Design option — absence-permissive temporal depth (pause-and-decay, not reset).** The reset rule above is the conservative choice; it penalizes ordinary intermittent use (a backgrounded or briefly-deactivated client), which is the common case for non-infrastructure users. A permissive variant supports intermittent use *without* weakening the rate limit, by separating the VDF's two jobs and unifying absence handling with the §6.1/§7.2 reputation decay model:
> - **Admission cost (one-time, anti-Sybil)** is absence-insensitive — a paid genesis chain stays valid; later absence does not undo it.
> - **Temporal depth (sustained presence, anti-laundering)** *pauses* on absence rather than resetting. Run the chain as freshness-windowed checkpoints `vdf_proof_k = VDF_eval(H(vdf_proof_{k-1}, beacon_{t_k}), δ_identity)`, where a checkpoint is on-chain-acceptable only if `beacon_{t_k}` is within a recency window `W_fresh`. On return after a gap the node resumes from its last checkpoint anchored to the *current* beacon (one step's work) — it neither recomputes the gap nor loses prior depth.
> - **Effective depth decays slowly during absence** (rate `δ_absence`, the absence analogue of `Δ_rise`): a short gap costs ≈ nothing; long dormancy erodes depth toward zero.
>
> Every anti-abuse property is preserved: sequentiality (per-identity, no parallel speedup) and the chain–identity binding above are unchanged; the freshness window blocks precompute **and** backfilling a gap with now-historical beacons (so identity age cannot be fabricated — OQ-57); and the slow decay caps the value of stockpiled dormant "sleeper" identities — the one abuse a non-resetting pause would otherwise open — at the same tradeoff already accepted for reputation (§6.1 absence-as-decay, not violation). Verification is unaffected: a Wesolowski/Pietrzak proof verifies in one group exponentiation regardless of gaps, and `effective_depth(T)` stays publicly recomputable from on-chain checkpoints + public beacons + the decay rule. A node only runs the VDF while online (a short `δ_identity`-sized step per checkpoint interval — a backgrounded app does periodic bursts, not a continuous grind), and the demand scales with role: casual clients need admission + light upkeep, whereas sustained depth is required only of mix/validator candidates (the dedicated high-uptime subset, §5.1.1). Settling reset vs. pause-and-decay, and calibrating (`W_fresh`, `δ_absence`), is the permissive-end refinement of **OQ-18**.
...
> **Scope of this section's protection.** Everything above protects the **gossip channel**. PSI handshakes (§5.4) are a separate, deliberate disclosure channel: a completed PSI reveals intersection information to the counterparty with no noise, no permutation, and no DP budget. The §1.7 P2 game does not cover PSI outputs; the leakage is bounded operationally (k_min §7.4, cache decay §4.8, rate limits §5.4), not information-theoretically (§8.1, OQ-64).
...
```
// ── OPTIONAL 2-of-2 PROFILE ONLY — not built under the adopted publish-s₁ design (box above) ──
...
```
// ── OPTIONAL 2-of-2 PROFILE ONLY — adopted design reads s₁ directly from public commit_T ──
...
Visual recap of §4.9.6, **adopted publish-`s₁` flow** — no new content; included for orientation only.
...
Reveal phase (slot i):
    ...
    Reveal is a verdict VOTE (no decryption share) — checked against the commit
    │
    ▼
Quorum (⌊K_committee/2⌋+1 SUSPEND reveals) within window?
    │                              ──── NO ────▶ i ← i + 1   (advance to next fallback)
    │                                            │
    │                                            ├─▶ if i < N_fallback : restart commit phase against committee_T^{(i)}
    │                                            └─▶ else : escalate to recursive oversight (§4.9.8)
    │ YES
    ▼
Validator finalization (in-block, canonical, SUSPEND verdicts only):
    σ_T^VERDICT = threshold-sign_VA("VERDICT_FINALIZED epoch_id_T")
    s₂          = VerEnc.Decrypt(d_T, σ_T^VERDICT)
    null_v      = s₁ + s₂   (mod p)            // s₁ read from public commit_T
    │
    ▼
null_v_decryption transaction submitted (by anyone, permissionless):
    dec_nullifier = Poseidon(verdict_hash, null_v)
...
*Optional 2-of-2 profile only (§4.9.4):* the reveal additionally carries a BLS share; after quorum anyone aggregates `σ_i^SUSPEND` and recovers the committee half `s₁ = ForwardCommit.Decrypt(c_T^{(i)}, σ_i^SUSPEND)` — validator finalization then releases `s₂` exactly as above. Under the adopted design no such aggregation step exists.
...
- **Transition criterion (normative — this is the predicate P1–P5 are conditioned on).** The *choice* of initial set is a deployment recommendation, but the criterion at which its privileged status sunsets is **not** — because §1.7 conditions every security property on it, it must be a measurable, publicly-verifiable predicate, not a judgment call. The transition fires when **all** of the following hold continuously over a confirmation window of `W_transition` epochs, each measured from public chain data (temporal depth, RouterInfo AS, and mix-selection weight are all public or recomputable, so the transition is self-verifying, not committee-attested):

  ```
  TRANSITION_READY ⟺  (a) consortium-redundancy ∧ (b) AS-diversity floor ∧ (c) pool-size floor,
                       all held for ≥ W_transition consecutive epochs.

  Let  E        = mix-eligible pool (nodes meeting the §5.1.1 mix-role uptime/bandwidth bar
                  AND VDF temporal depth ≥ τ_depth),
       w(·)     = mix-selection weight (∝ advertised honest bandwidth),
       W_org    = Σ w over ORGANIC (non-consortium) members of E,
       W_cons   = Σ w over consortium members of E,
       f_Loopix = max adversarial mix fraction the deployment's Loopix parameters tolerate.

  (a) Consortium-redundancy.   W_cons / (W_org + W_cons)  ≤  f_Loopix.
       The consortium's share has fallen to within the adversarial fraction Loopix already
       tolerates: even if the *entire* consortium turned adversarial post-sunset, A2 in the
       mix pool still holds on the organic majority alone. The trusted set is now redundant,
       not load-bearing.

  (b) AS-diversity floor.       max_AS share of W_org ≤ s_AS_max   AND   #distinct ASes ≥ A_min,
       chosen so the per-path full-compromise term f^L (SECURITY.md §P1.1) stays ≤ the target
       ε_loopix for path length L — i.e. "f_Loopix tolerated" is meaningful because no single
       AS or small AS-coalition can dominate a sampled path.

  (c) Pool-size floor.          |E| ≥ N_min,  with honest-mix pool  n̄ = λ_mix/μ ≥ n̄_min,
       so the per-hop anonymity set supports the target sender-anonymity (ties to OQ-58 λ).
  ```

  Conservative placeholder constants (`τ_depth`, `f_Loopix`, `s_AS_max`, `A_min`, `N_min`, `n̄_min`, `W_transition`) are deployment/Phase-5 calibration — the **empirical residual of OQ-12**. The *shape* above (organic redundancy of the consortium, gated by AS-diversity and pool-size floors, sustained over a window, publicly recomputable) is the normative part; the constants are not pinned here.
- **Honest bootstrap framing.** Until `TRANSITION_READY` holds, the Loopix anonymity guarantee for the deployment is **conditional** on the bootstrap consortium and should be advertised that way. This is no worse than how Tor's and Nym's mix anonymity bootstraps in practice — both rely on a real set of operators whose honesty is externally evaluated — but PrivaCF should be explicit about the bootstrap dependency rather than treating A2 as self-evidently satisfied at deploy time.

The protocol makes no assumption about *how the initial set is chosen* (a deployment recommendation); the **transition criterion above is normative**, because the §1.7 security argument is conditioned on it. §1.7 (composition / "two halves load-bearing") and §2.1 (genesis bootstrap) reference this subsection as the single home for the conditionality rather than restating it.
...
What PSI does **not** protect: the intersection itself is revealed to the initiator on success, and even a score-only result supports adaptive membership probing (vary the probe set across runs, observe score deltas). Repeated PSI by a coordinated set of counterparties therefore accumulates un-noised knowledge of a target's item set — a channel **outside the §1.7 P2 guarantee** and outside the §4.5 DP budget. Mitigations are operational only: the k_min cluster-behavior gate (§7.4), PSI cache decay (§4.8) limiting re-runs, and the `n_discovery`/`n_psi_in` rate limits above. No formal bound on cumulative PSI leakage is claimed (§8.1, OQ-64).
...
- `α` is the blend weight between current score and historical consistency. (It is unrelated to the per-node trust cap fraction in §7.3, which an earlier draft also called `α` and which is now named `f_cap`.)
...
#### 6.2.1 Behavioral-label k-anonymity under Loopix (cross-epoch linkage)

The published per-node label is a privacy liability of its own, distinct from its use as a diversity axis. Because the label is a deterministic, publicly-recomputable function of public chain data (above), a node whose fingerprint is stable carries the **same label across epoch-`id` rotations** — a standing cross-epoch quasi-identifier layered over the per-epoch `epoch_id`. §8.1 records this qualitatively ("behavioral fingerprint as persistent identifier"); this subsection quantifies how identifying it is under the thin Loopix fingerprint, and states the result against P1 (§1.7).

**Effective anonymity set.** For a node `v` with label `ℓ(v)` computed over a re-clustering window `w` (population `N_w` = nodes with an `epoch_transaction` that window), define the cross-epoch behavioral anonymity set

```
A_eff(v) = | { u ∈ N_w : ℓ(u) = ℓ(v) } |.
```

An adversary linking `epoch_id_T` to `epoch_id_{T+k}` using only the published label rules out every node not sharing `ℓ(v)`, shrinking the candidate set from `N_w` to `A_eff(v)`. Two regimes bound it:

- **Uniform (best case).** Equal cluster masses give `A_eff = N_w / k_cluster`.
- **Skewed (realistic).** The Loopix fingerprint is degenerate as a partitioning signal. `epoch_presence` saturates near 1 for *any* active node (active nodes publish most epochs), so it carries almost no signal; the only live discriminator is `audit_response_rate`, itself concentrated near 1 for honest nodes answering most challenges. The population collapses into a dense modal blob plus a sparse tail. k-means over this puts almost all mass in one or two large clusters and assigns the tail — nodes with atypical presence or low audit-response — to small clusters, so `A_eff` for a tail node can be `O(1)–O(10)` *even when* `N_w / k_cluster` is large. Tail membership is exactly the set of behaviorally-distinctive nodes, so the channel bites precisely where it matters — the standard k-anonymity outlier problem.

**Community-size dependence.** `A_eff` scales with `N_w`: in a niche deployment or a thin epoch (small `N_w`), even the modal cluster is small and the whole partition offers little cover. The behavioral channel's protection is weakest exactly where the network is smallest — the same regime where A2 and Loopix cover are also weakest, so the weaknesses **compound rather than being independent**.

**Bound against P1.** The behavioral-channel contribution to the P1 unlinkability advantage is, conservatively,

```
ε_cluster(v, T, k) ≤ p_stable(v, k) · (1 − A_eff(v) / N_w)
```

where `p_stable(v, k)` is the probability `v`'s label is unchanged between the two re-clusterings (fingerprint stability over `k` epochs). For a stable outlier (`p_stable → 1`, `A_eff → 1`) this approaches 1 — the label alone effectively links the node across epochs. It is **additive to and independent of** the transport term `ε_Π`, because it leaks from published chain data, not the wire: cover traffic does not bound it.

**Intrinsic ceiling — why label-side mitigations are capped.** The label is a *generalization* (coarsening) of the raw fingerprint, so `A_eff` is always **at least** the anonymity set of the raw two-feature fingerprint — which is itself publicly recomputable from chain data (`epoch_presence`, `audit_response_rate` are both public). Suppressing or ZK-hiding the published label therefore cannot raise `A_eff` above the raw-fingerprint anonymity set; an observer just recomputes the fingerprint directly. This caps how much any committee-side mitigation can buy and is the quantified form of the §8.1 persistent-identifier residual.

**Mitigations.** Three raise the *floor* without touching the ceiling; one touches the ceiling:

1. **k-anonymity floor on publication.** The committee merges/withholds any label class with `|ℓ| < k_floor`, publishing a coarsened partition so no node sits in a sub-`k_floor` class. Guarantees `A_eff ≥ k_floor` for the *published* channel; does not stop raw-fingerprint recomputation.
2. **Coarser `k_cluster` under Loopix.** Fewer, larger classes directly raise `N_w / k_cluster`, trading diversity-axis resolution (already dropped under Loopix, above) for anonymity.
3. **Drop per-node labels under Loopix.** Publish only centroids + ZK cluster-membership predicates (§6.2 OQ-11 path 2); nodes prove cluster predicates in zero knowledge when a diversity check needs one, so no standing per-node label sits on chain. Removes the published quasi-identifier, not the recomputable fingerprint.
4. **Fingerprint coarsening at the source (touches the ceiling).** Publish `audit_response_rate` bucketed rather than as a fine rate, and report `epoch_presence` over a longer window — shrinking the raw-fingerprint *cell count* itself, the only lever that raises the intrinsic ceiling, at a cost to audit-signal fidelity.

**Net, against P1.** Under Loopix the behavioral label is a standing cross-epoch quasi-identifier **outside P1's PRF+transport bound**, with effective anonymity set `A_eff` governed by `min(k_cluster occupancy, raw-fingerprint cell count)` and binding in the tail and in small communities. It is bounded operationally by a `k_floor` publication rule and source-side fingerprint coarsening, **not** information-theoretically — the same disclosed-channel posture as the PSI carve-out (OQ-64). Quantification and per-config calibration of `A_eff`, `k_floor`, and the audit-fidelity cost is **OQ-65**.
...
committee_T = VRF(beacon_T ‖ "audit_committee" ‖ epoch_id_v,
                  ...
                  constraints = [
                      different_interest_clusters,
                      different_behavioral_clusters,   // Tor/I2P profile only; under Loopix the behavioral
                                                       // axis is too thin and is replaced by the mix-layer
                                                       // AS/density diversity axis + larger K_committee
                                                       // (§6.2 "Auditor independence", §6.3, exp 5.49)
                      reputation ≥ median,
...
**Per-event ε-DP — a rate bound, not an isolation guarantee.** Laplace noise in §4.5 is parameterized by ε. Each gossip event leaks at most ε of preference information; this is a **per-event** bound only. Cumulative loss over T epochs grows as Tε (or ≈ √(2T ln(1/δ'))·ε under advanced composition) and is **not bounded over a node's lifetime** — lifetime DP is explicitly not claimed (§8.1 "Long-term differential privacy"). What the per-event bound buys against Sybils is therefore *rate*, not impossibility: a Sybil ring inside a node's PSI neighborhood must accumulate observations across many epochs before re-identification from gossip output becomes feasible, and the `k_min` gate above plus PSI cache decay (§4.8) limit how quickly a ring can position itself to collect those observations. Note also that this bound covers the **gossip channel only** — PSI outputs are un-noised by design and sit outside it entirely (§4.5 scope note, §8.1, OQ-64). See OQ-55 for the formal per-event accounting under the SecLDP framework.
...
| **B** | **Behavioral-probabilistic — intended, empirically unverified until Phase 5.** Detection rate is governed by a calibrated threshold; false positive and false negative rates are *to be* characterized empirically. **A B label asserts a calibratable detector, not a demonstrated detection rate.** Every B row's strength rests on the per-flag raise probabilities `p(flag_i \| S)` of §7.1b, which have **no first-principles values and are not derived** — they are filled in only by the Phase 5 experiment named in the row's OQ. Read B rows as "intended, empirically unverified," not as derived guarantees. |
...
**Until those experiments run, every B-, H-, and PARTIAL-level row is unvalidated** — the detection strengths in the table are *intended* behavior whose false-positive/false-negative rates rest on the uncalibrated `p(flag_i | S)` constants of §7.1b, not measured results. Only the C-level rows (11, 14, 18 — detection by arithmetic) hold independent of Phase 5. Each B-level, H-level, and PARTIAL row in the table corresponds to at least one Phase 5 experiment in §9.2 (with the exception of self-harming rows like row 20, where TPR/FPR characterization is not meaningful — the attacker's own CF quality degrades and operator action is unnecessary). The detection contract above can be used as a checklist: for each non-self-harming row marked B, H, or PARTIAL, the corresponding experiment must produce a TPR/FPR characterization with an explicit calibration recommendation (for PARTIAL rows, the characterization must also state the content-irreducible residual and which non-content axis bounds it). Rows marked O are not eliminated by Phase 5 — they are documented residuals.
...
- **PSI intersection leakage — outside P2.** The P2 indistinguishability game (§1.7) covers the gossip channel only. PSI handshakes reveal intersection information to the counterparty un-noised, and a coordinated set of adversarial peers running repeated PSI against a target reconstructs a growing fraction of the target's item set by union over runs (adaptive membership probing works even on score-only results). The interest-cluster peers with the highest PSI hit rate are also the most informative counterparties, so this compounds the within-cluster tradeoff above. Mitigated operationally (k_min §7.4, PSI cache decay §4.8, `n_discovery`/`n_psi_in` rate limits §5.4); no information-theoretic bound is claimed (OQ-64; CRYPTO.md "What it does not protect against").
- **Long-term differential privacy.** T epochs of ε-DP gives Tε total loss. Not claimed. (Per-event ε bounds the *rate* of gossip-channel leakage only — §4.5, §7.4.)
...
- **Behavioral fingerprint as persistent identifier.** Derived from public on-chain data and available to any observer. Degraded but not eliminated by per-n-epoch commits, relay submission, and transaction timing jitter. **Quantified in §6.2.1** as the `ε_cluster` term in P1: the published cluster label has effective anonymity set `A_eff = min(k_cluster occupancy, raw-fingerprint cell count)`, which collapses to `O(1)–O(10)` in the behavioral tail and in small communities; label-side mitigations (`k_floor` publication, ZK predicates) raise the floor but are capped by the publicly-recomputable raw fingerprint, and only source-side coarsening touches that ceiling. Calibration is OQ-65.
...
> **Pre-Phase-1 feasibility gate (P-feasibility — resolve before building this phase).** The "Statement 5 proof generation and verification end-to-end verified" exit criterion below is **not** a routine Phase-1 deliverable: it presumes the **desktop Statement-5 proving benchmark has already passed** as a separate, earlier gate. The circuit *as originally specified* (in-circuit ForwardCommit pairing) is AMBER→RED — ≥99% of constraints, 2–13 M total ([SPIKE-statement5.md §8](./SPIKE-statement5.md#8-phase-0a-result--constraint-estimate-2026-06-06)) — and a bad benchmark **changes the design, not the parameters** (§1.7 substrate-contingency gate, §10.1.1 V1 P-feasibility). The build target is therefore the **restructured publish-`s₁` circuit** (public `s₁` + `d_T` verifiable-encryption *bridge* binding + SMT non-membership, **no in-circuit pairing**), whose estimated 0.3–1 M constraints are the AMBER range that must clear the desktop benchmark before the rest of Phase 1 proceeds. Treat this gate as foregrounded, not folded into the exit list. **The bridge has now been measured ([SPIKE §10](./SPIKE-statement5.md#10-phase-1b-result--the-bridge-measured-packing-factor-collapsed-2026-06-16), `impl/spike_bridge_cost/`): the single non-native BLS12-381 `G₁` Pedersen opening it needs is ~2²¹ trace rows ≈ ~2 min desktop with a naive gadget (RED), ~5–40 s with a purpose-built one (AMBER) — heavier than the prior "sub-second" lean.** Concretely this makes the **optimized non-native bridge gadget a Phase-1 deliverable with its own tracked exit criterion** (target ≤ ~2²⁰ rows / ≤ ~30 s desktop): build the real gadget, read its `degree_bits`, confirm AMBER before committing the rest of Layer-1. The core (everything but the bridge) is measured GREEN (§9) and unaffected.
...
5.43 DKG/PSS timing constraint — epoch_transaction submission deadline (2-of-2 profile) and VA_pub PSS re-share window (adopted profile) calibration
...
| OQ-12 | What is the minimum genesis set size for bootstrap viability, and at what point can the behavioral cluster diversity constraint be enforced without relaxation — equivalently, when does the externally-trusted mix consortium's privileged status sunset?                                    | Network          | empirical                | 3    | 3      | partial — **transition criterion now defined normatively (§5.1.1):** `TRANSITION_READY` = consortium-redundancy (`W_cons/(W_org+W_cons) ≤ f_Loopix`) ∧ AS-diversity floor ∧ pool-size floor, held over `W_transition` epochs, all publicly recomputable from chain data. The *shape* is pinned; the constants (`τ_depth, f_Loopix, s_AS_max, A_min, N_min, n̄_min, W_transition`) remain the empirical residual. The genesis set floor is bounded by interest-cluster committee viability (not dual-cluster coverage), since the recommendation layer remains useful during the behavioral-cluster-relaxed bootstrap phase via content-based cold-start (§13); the behavioral-cluster enforcement threshold is the cluster-density analogue of the §5.1.1 temporal-depth-mass condition. See OQ-44 for the tightening schedule.                                                                                                                                                                                                          |
...
| OQ-18 | What is the correct VDF gap tolerance policy — at what point should a partially completed chain be invalidated vs. allowed to resume?                                                                                                                                                       | Identity         | empirical                | 3    | 2      | starting value set (§10.1.1 V1 build parameters); refined Phase 5                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| OQ-26 | What auditor committee size K_committee minimizes collusion probability while remaining viable under cluster diversity constraints?                                                                                                                                                         | Reputation       | empirical                | 4    | 3      | starting value set (§10.1.1 V1 build parameters); refined Phase 5                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| OQ-27 | What validator set size K_validators correctly balances BFT safety margin against participation overhead?                                                                                                                                                                                   | Network          | empirical                | 3    | 2      | starting value set (§10.1.1 V1 build parameters); refined Phase 5                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| OQ-30 | What is the correct maximum frame size across all protocol message types, including the PSI acknowledgment message type?                                                                                                                                                                    | Network          | empirical                | 1    | 1      | starting value set (§10.1.1 V1 build parameters); refined Phase 5                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| OQ-50 | What DKG completion deadline correctly constrains the epoch_transaction submission window without creating liveness pressure on slow committee members?                                                                                                                                     | Network          | empirical                | 3    | 2      | open — **2-of-2 profile only** under the adopted publish-`s₁` design (§4.9.4: committees run no per-epoch DKG); for the adopted profile the analogous deadline is the `VA_pub` PSS re-share window (§4.1, §8.2 T8) — starting values set in §10.1.1 (W_reshare=3 epochs, threshold=21) |
...
| OQ-58 | What base Poisson rate λ for Loopix loop and drop cover traffic provides sufficient anonymity against a mix-node-observing adversary without imposing unacceptable bandwidth overhead on metered or mobile deployments, and what is the correct λ per config (A–E)?                         | Network          | empirical                | 3    | 2      | **analytical pre-bound DERIVED (keystone)** — `ε_loopix ≤ f^L + (1−f^L)·ρ_SDA`, `ρ_SDA ≤ C_SDA·m_obs/(n̄·(1+r))`, `n̄ = λ_mix/μ`, written out in [SECURITY.md §P1.1](./SECURITY.md#p11-conservative-ε_loopix-pre-bound-oq-58). The `f^L` (full-path-compromise) term is rigorous and sizes path length `L`; only the `O(1)` SDA constant `C_SDA` and the per-config `λ` remain. Unblocks numerical evaluation of OQ-19/46/55. Phase 5 exp 5.51 pins `C_SDA` and refines λ per config A–E rather than deriving the bound from scratch. See §10.1.1. **2-of-2-profile coupling (deployment-conditional):** under the optional high-assurance 2-of-2 profile only, λ also has a *lower* bound from the opposite direction — per-epoch committee DKG must push `≈(N/E)·F·K²·R` messages through the fixed-rate Loopix sender inside the §4.1 DKG window, and that throughput floor competes with cover traffic for the same bandwidth ([ANALYSIS-dkg-load.md §8](./ANALYSIS-dkg-load.md#8-sim-result-2026-06-06)). So if 2-of-2 is ever deployed, λ must satisfy *both* the anonymity lower bound (here) and the DKG-throughput lower bound jointly, and cohort-sharing (option B) is what keeps that joint constraint feasible. The adopted publish-`s₁` profile has no per-node DKG, so this coupling does not arise in the default. |
...
| OQ-64 | Can cumulative PSI intersection leakage against a coordinated adversarial peer set be formally bounded — e.g., via noised/cardinality-only PSI variants, per-counterparty query budgets, or DP-PSI — without degrading peer-selection quality below θ_cluster usefulness? (The channel is currently outside P2 and bounded operationally only — §1.7, §4.5, §5.4, §8.1.) | Privacy | theoretical → empirical | 2 | 3 | open |
| OQ-65 | What is the effective cross-epoch anonymity set `A_eff` of the published behavioral cluster label under Loopix, as a function of `k_cluster`, population `N_w`, and the honest-node fingerprint distribution — and what `k_floor` publication rule and source-side fingerprint coarsening hold the `ε_cluster` term in P1 (§1.7, §6.2.1) below a target re-identification rate without unacceptably degrading audit-signal fidelity? | Privacy | theoretical → empirical | 2 | 3 | **partially resolved** — anonymity-set structure derived analytically (§6.2.1): `A_eff ≥` raw-fingerprint anonymity set (label is a generalization), `ε_cluster ≤ p_stable·(1 − A_eff/N_w)`, additive to and independent of `ε_Π`; the binding regimes (behavioral tail, small `N_w`) and the intrinsic ceiling (publicly-recomputable raw fingerprint caps label-side mitigations) are identified. Remaining: simulate `A_eff` over honest-fingerprint distributions per config, pin `k_floor`, and quantify the audit-fidelity cost of source-side coarsening (Phase 5). |
...
- **P-feasibility (substrate gate) — *hard release gate*.** None of P1–P5 (§1.7) is creditable until (a) Layers 1–4 are built through Phase 3 and (b) the **desktop** Statement-5 circuit hits its proof-generation benchmark (= the desktop half of OQ-3, already a Phase-3 PoC gate). A bad answer changes the design. The PoC to date is Layer 5 only with crypto modeled in the clear; this gate makes that boundary explicit rather than implicit. Tracked in [SECURITY.md](./SECURITY.md#summary--status-after-this-companion). **Phase-0 estimate done** ([SPIKE-statement5.md §8](./SPIKE-statement5.md#8-phase-0a-result--constraint-estimate-2026-06-06)): Statement 5 *as specified* is AMBER/RED — the in-circuit ForwardCommit pairing is ≥99% of constraints — and must be restructured (move BF-IBE ciphertext-well-formedness to a public publish-time validator check; keep only the cheap `s₁+s₂=null_v` Poseidon binding in ZK) before the build. Not a dead end; a "redesign one component first" gate. **Phase-1 core proving measured GREEN (~30 ms, [SPIKE §9](./SPIKE-statement5.md#9-phase-1-proving-result--the-publish-s₁-core-measured-2026-06-07)) and the bridge measured (`impl/spike_bridge_cost/`, [SPIKE §10](./SPIKE-statement5.md#10-phase-1b-result--the-bridge-measured-packing-factor-collapsed-2026-06-16)):** the single non-native BLS12-381 `G₁` Pedersen bridge opening is ~2²¹ rows ≈ ~2 min naive (RED) / ~5–40 s tuned (AMBER) — heavier than the prior estimate, so the **optimized bridge gadget is a tracked Phase-1 exit criterion** (≤ ~2²⁰ rows / ≤ ~30 s desktop), not a near-free term. Gate verdict: passable, conditional on that gadget.
...
**V1 — build parameters (need a build-time starting value for Phase 1–3).** The five parameters below are now initialized to concrete **Phase-1 starting values** — normative defaults that let the code be written and the substrate stood up. Each is *refined* (not redefined) by its mapped Phase-5 experiment; none is a gate. Phase 1–3 code is written against these.

| OQ | Parameter | Phase-1 starting value | Rationale · refinement |
|---|---|---|---|
| OQ-30 | `frame_size` — fixed Sphinx payload, all mixnet message types | **2 KiB payload** (+ Sphinx header); a single fixed size, messages over it fragment into independently-padded frames | A single fixed frame is what makes message *types* indistinguishable (§5.1); it must hold the largest single mixnet message (a PSI round element-batch, or a gossip push carrying MinHash + epoch_id). Refine against the measured message-size distribution in Phase 3 — the `psi_ack` type is the tight case (OQ-61). |
| OQ-18 | `G_vdf` — VDF chain gap tolerance | **resume if ≤ 1 epoch of checkpoints is missing; invalidate (force re-genesis) beyond that** | One VDF checkpoint per epoch anchored to the beacon (§4.3); a single missed checkpoint may be recomputed and re-anchored, a longer gap re-seeds from genesis (forfeiting accrued identity age). Conservative against laundering — a node cannot bank VDF work then resurface (OQ-57). **Permissive alternative in §4.3 (design option):** freshness-windowed checkpoints + pause-and-decay of temporal depth (no re-genesis), which supports intermittent/backgrounded use without weakening the rate limit — the likely refinement if the honest-offline distribution shows the reset rule is too harsh. Refine against the observed honest-offline distribution (Phase 5). |
| OQ-27 | `K_validators` — BFT validator set size | **31** (tolerates `f = 10`; BFT bound `K ≥ 3f+1`); odd size avoids split votes | Honest-majority margin over A2. The `VA_pub` attestation threshold is pinned at the BFT quorum `⌊2K/3⌋+1 = `**`21`** — required by the publish-`s₁` sign-off ([DESIGN §10](./DESIGN-f1-verifiable-encryption.md#10-threat-model-sign-off-for-publish-s₁-phase-1-step-2)): covert `null_v` recovery must need > 2/3 validator collusion, which is beyond A2 and slashable. Refine vs. participation overhead (Phase 5). |
| OQ-26 | `K_committee` — auditor committee size | **21**; verdict-vote threshold `⌊K/2⌋+1 = `**`11`** | Sized from the §4.9.8 Chernoff compromise bound `exp(−K·D(½‖q))` and the `q_eff(Loopix)` scaffold ([SECURITY.md](./SECURITY.md#committee-compromise-scaling--q_effprofile-exp-549-scaffold), Loopix ≈ 2.4× the Tor set: ~9 → ~21). Matches the `ANALYSIS-dkg-load.md` cost model. Refine vs. the `q` estimate (exp 5.49). |
| OQ-50 | `W_reshare` — `VA_pub` PSS re-share window; VA threshold | **`W_reshare = 3` epochs**; threshold = BFT quorum **21** (= OQ-27) | Highest-criticality single point of liveness (§8.2 T8): a failed re-share strands the `s₂` half of every pending `commit_T`. Window set ≫ the PSS round latency (`ANALYSIS-dkg-load.md`: one threshold-key protocol ≈ 2.7 min over the mixnet, far inside 3 epochs), so the stranding probability stays low across rotation; `VA_pub` is constant, only the shares are PSS-refreshed (§4.1). The 2-of-2-profile committee-DKG deadline (the other half of OQ-50) follows the same rule but applies only if that optional profile is deployed. Phase 5 only refines the window against measured rotation behavior. |

The four committee/validator/window values are internally consistent: `K_validators = 31 → VA threshold ⌊2·31/3⌋+1 = 21`; `K_committee = 21 → verdict-vote threshold ⌊21/2⌋+1 = 11`.

**V1 — calibration (Phase 4–5 simulation; in scope, not blocking).** Each maps to a Phase-5 experiment: reputation/CF (OQ-9, 20, 21, 23, 24, 25, 38, 41, 42, 45, 54); Sybil thresholds (OQ-28, 29, 31, 32, 34, 35, 43, 44, 48, 59); privacy params (OQ-19, 37, 46, 51, 65); liveness/infra (OQ-12, 39, 40, 52). OQ-55's DP formalization is needed only to state P2's `ε` explicitly; otherwise deferrable.
...
- **Honest-disclosure hardening:** OQ-64 (formal bound on cumulative PSI intersection leakage — the channel is disclosed as outside P2 (§1.7, §8.1) and operationally limited; a DP-PSI or query-budget construction would upgrade the disclosure to a bound, but the PoC ships with the carve-out stated, not closed).
...
// PSI REQUEST (interest-tier candidate handshake — §5.4 Step 3, Appendix I)
{
  "type":        "psi_request",
  "epoch_T":     "<T>",
  "psi_payload": "<asymmetric PSI first-round message (Pinkas et al. 2018) — OT-extension encodings over the initiator's item set; reveals nothing absent protocol completion>",
  "surb":        "<single-use reply block for psi_ack + PSI result — the responder never learns the initiator's identity or return address (Loopix)>"
  // Deliberately unauthenticated: no initiator epoch_id or signature, preserving initiator
  // pseudonymity toward the responder (§5.4). Abuse is bounded by the responder-side
  // n_psi_in silent-drop limit, not by sender accountability.
}
...
EACH EPOCH (offset_v stagger)
  ...
  ├── Split null_v = s₁ + s₂ (mod p); construct commit_T = ( s₁,        // published in clear (adopted publish-s₁)
  │       d_T = VerEnc(s₂, "VERDICT_FINALIZED epoch_id_T", VA_pub; r_commit_d_T) )
  │   [2-of-2 profile only: await committee DKG → threshold_BLS_pk_T^{(i)}, then commit_T instead = (
...
TIER 1 — Build now
  ...
  VA_pub standing validator key    Bootstrap DKG once + PSS re-share at validator rotation; blst crate
                                   (committees run NO per-epoch DKG under adopted publish-s₁ — §4.9.4;
                                    per-epoch committee DKG is 2-of-2 profile only)
  Verdict_commit transaction type  Commit phase; H(verdict ‖ nonce)  [2-of-2 profile adds bls_share]
  Verdict_reveal transaction type  Reveal phase (verdict vote); match against commit
  null_v_decryption transaction    Permissionless; gated on σ_T^VERDICT; s₁ from public commit_T; SUSP_SMT insertion
  Watchdog signal                  Public chain rate monitoring
  Statement 5 circuit              Poseidon checks + s₁+s₂ split (s₁ public input) + VerEnc(s₂) binding
                                   + SMT non-membership (Plonky3, no in-circuit pairing — §4.9.5)
...
CONFIG E — Binary Ratings (strongest DSybil transfer — heuristic, OQ-10)
  Transport:      Loopix/Sphinx + Dandelion++ (broadcast only)
  Identity:       Poseidon PRF rotation
  Noise:          Chopping + cover items
...
| DSybil non-overwhelm.¹  | 1   | 1   | 2   | 2   | 3   |
...
_¹ **DSybil non-overwhelming is a heuristic transfer, not a formal guarantee, in every config (§7.3, OQ-10).** The DSybil theorem (Yu et al.) requires a persistent social graph, transitive trust propagation, and a sparse honest/Sybil cut — none of which PrivaCF has — so its formal bound does not transfer; the rule is retained as a well-motivated heuristic with empirical support from the original Digg validation. The column is therefore scored as **relative transfer strength, capped below "formal" (max 3)**, not as a proof level. CONFIG E (binary ratings) scores highest because binary feedback is exactly the setting in which DSybil was validated, narrowing — but not closing — the gap; continuous-rating configs additionally depend on the unproven per-epoch composition lemma (OQ-10). A formal bound in PrivaCF's setting requires live-deployment data (OQ-10, §7.3 "Caveat on formal transfer"). The "Formal DP guarantee" row is unaffected: CONFIG D's ε-DP is genuinely formal (clean ε-DP by post-processing under the §4.5 clamp mechanism, §1.7 P2)._
...
```
   ...
   │  No DKG / no threshold key (adopted publish-s₁) —    │
   │  aggregate multisig for handoff acceptance           │
   │  (2-of-2 profile only: per-epoch DKG → threshold pk) │
   │  Shamir share of encrypted state chain               │
   │  Commit-reveal verdict VOTE for suspensions          │
   ...
   │  Verify commit_T ZK proofs                           │
   │  Hold standing VA_pub (PSS re-share at rotation)     │
   │  Publish σ_T^VERDICT on SUSPEND finalization         │
...
## Appendix J — Future Directions: Decentralized Learning Profile

_Non-normative. This appendix stakes a scope claim about the substrate; it specifies no protocol and adds no V1 requirement. Relocated here from the normative section range in v0.3.1 so the implementation-focused core carries only what the PoC builds._
...
A full DL profile is out of scope for v0.3.1; the goal here is to make the substrate's scope explicit and to record what would be required to host a DL workload.

### J.1 Why DL fits the substrate
...
### J.2 What the substrate would need to add
...
### J.3 Privacy and Sybil properties under a DL profile
