...
| OQ-18 | `G_vdf` — VDF chain gap tolerance | **resume if ≤ 1 epoch of checkpoints is missing; invalidate (force re-genesis) beyond that** | One VDF checkpoint per epoch anchored to the beacon (§4.3); a single missed checkpoint may be recomputed and re-anchored, a longer gap re-seeds from genesis (forfeiting accrued identity age). Conservative against laundering — a node cannot bank VDF work then resurface (OQ-57). Refine against the observed honest-offline distribution (Phase 5). |
