...
| OQ-50 | What DKG completion deadline correctly constrains the epoch_transaction submission window without creating liveness pressure on slow committee members?                                                                                                                                     | Network          | empirical                | 3    | 2      | open — **2-of-2 profile only** under the adopted publish-`s₁` design (§4.9.4: committees run no per-epoch DKG); for the adopted profile the analogous deadline is the `VA_pub` PSS re-share window (§4.1, §8.2 T8)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
**V1 — build parameters (need a build-time starting value for Phase 1–3).** OQ-30 (frame size), OQ-18 (VDF gap policy), OQ-50 (VA_pub PSS re-share window; committee-DKG deadline under the 2-of-2 profile only), OQ-27 (K_validators), OQ-26 (K_committee). The last four are refined by Phase-5 calibration but must be initialized to implement the code.
