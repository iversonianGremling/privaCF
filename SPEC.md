...
| `commit_T`          | Forward-secure nullifier commitment: N_fallback committee ciphertexts of share `s₁` plus the validator ciphertext `d_T` of share `s₂` | §4.9.4 |
| `s₁`, `s₂`          | Additive shares of `null_v` over the scalar field: `s₁ + s₂ = null_v (mod p)`; `s₁` → committee, `s₂` → validators | §4.9.4 |
...
**The dark node rotator** earns a SUSPENDED verdict and goes offline before their nullifier is extracted, then attempts to re-admit from the same key. Defended by the forward-secure commitment: on a finalized verdict, the committee share (`s₁`) and the validator verdict attestation (`s₂`) together reconstruct `null_v` without node cooperation (§4.9.4). Because `null_v` is split and the validator share is released only by a public verdict, neither a present nor a future committee compromise can extract it covertly. The residual gap — nodes that go dark during the admission window before publishing any `commit_T` — is bounded by zero reputation and behavioral fingerprinting.
...
**P4 — Forward-secure nullifier extractability.** Let `commit_T = (c_T^{(0..N−1)}, d_T)` be a node's commitment for epoch `T`, with `null_v = s₁ + s₂` split across the committee ciphertexts (`s₁`) and the validator ciphertext (`s₂`, §4.9.4). Recovery requires *both* a committee SUSPEND signature `σ_i^SUSPEND` and the validator verdict attestation `σ_T^VERDICT`:

```
Pr[anyone recovers null_v given both σ_i^SUSPEND and σ_T^VERDICT] = 1
Pr[anyone recovers null_v from a committee signature alone]       ≤ ε_IBE     (yields only s₁ ⟂ null_v)
Pr[anyone recovers null_v without σ_T^VERDICT]                    ≤ ε_BLS + ε_IBE
```

Reduces to threshold BLS unforgeability (Boneh-Lynn-Shacham) + Boneh-Franklin IBE security. Multi-recipient construction (§4.9.4) preserves both bounds with `N_fallback` independent committees; stalling-minority decay is exponential in `N_fallback`. **Forward secrecy of past `commit_T`** (a *future* compromise of a past committee's keys retroactively decrypting past `commit_T` without a verdict) is now closed cryptographically: a committee compromise yields only `s₁`, and `s₂` is released only by a validator verdict attestation that is itself public and slashable if forged (§4.9.4). Canonical treatment in §8.2 T12.
...
- **Laplace DP** (mainstream, formal guarantee): `gossip_v(T)[i] += Laplace(0, S/ε)` with `‖p_v‖₁ = 1` enforced and L1 sensitivity S = 2. Laplace DP does **not** by itself guarantee sign preservation — the noise distribution is unbounded. Sign preservation is an additional constraint applied as a separate constraint below, which slightly weakens the pure-DP guarantee.
...
**Problem:** `null_v` must be available for extraction after a suspension verdict even if the node is offline, without enabling covert extraction by the committee — and, critically, without enabling *retroactive* extraction by any future compromise of a past committee's keys (the forward-secrecy gap, §8.2 T12). The construction below splits the plaintext so that the committee never holds enough to recover `null_v`, and the missing half is released only by a public, accountable verdict.
...
**Statement 5 — Non-suspension and forward commitment** (Poseidon evaluation + SMT non-membership + ForwardCommit certification across all N_fallback slots):

```
null_v      = Poseidon(sk, "null_v")
epoch_id_T  = Poseidon(sk, beacon_T, null_v, "epoch_id")
null_v ∉ SUSP_SMT_root_T
s₁ + s₂ = null_v   (mod p)                       // additive split certified
∀ i ∈ {0, ..., N_fallback − 1} :
    c_T^{(i)} = ForwardCommit(s₁, "SUSPEND epoch_id_T",           threshold_BLS_pk_T^{(i)}; r_commit_T^{(i)})
d_T           = ForwardCommit(s₂, "VERDICT_FINALIZED epoch_id_T", VA_pub;                   r_commit_d_T)
```

All N_fallback committee ciphertexts share the same `s₁` wire, `d_T` carries `s₂`, and the circuit enforces `s₁ + s₂ = null_v` — so every (committee slot, `d_T`) pair recovers the *same* canonical nullifier, and a node cannot smuggle divergent or unrelated plaintexts into any slot. The `s₁`/`s₂` split is a private witness; the circuit reveals neither share. Extended Statement 5 circuit cost grows linearly in N_fallback (a pairing check per committee slot) plus one pairing check for `d_T` and one field-addition constraint for the split; mobile-hardware benchmarking required before deployment (OQ-3).
...
handoff_v(T) = {
    ...
    SUSP_SMT_root_T,
    commit_T = ( c_T^{(0)}, ..., c_T^{(N_fallback−1)},  d_T ),
    threshold_BLS_pk_T^{(0..N_fallback−1)},   VA_pub,
    ZK proof that:
        ...
        s₁ + s₂ = null_v                         (Statement 5, split)
        ∀ i : c_T^{(i)} = ForwardCommit(s₁, "SUSPEND epoch_id_T",
                                        threshold_BLS_pk_T^{(i)}; r_commit_T^{(i)})       (Statement 5)
        d_T = ForwardCommit(s₂, "VERDICT_FINALIZED epoch_id_T", VA_pub; r_commit_d_T)     (Statement 5)
...
| 14  | any × I8 post-publish     | Dark node, post-`commit_T`                  | On a finalized verdict, committee share `s₁` + validator attestation `s₂` reconstruct `null_v` from on-chain `commit_T` | §4.9.4 ForwardCommit (2-of-2 split); §4.9.6 commit-reveal          | C                                                                              | None — recoverable without node cooperation; covert recovery requires both a committee threshold and validator equivocation |
...
- **Threshold collusion for null_v extraction.** Under the 2-of-2 split (§4.9.4), a colluding committee supermajority that aggregates `σ^SUSPEND` off-chain now recovers **only the committee share `s₁`** — which is independent of `null_v` without the validator share `s₂`. To recover `null_v` covertly the adversary must *additionally* obtain `σ_T^VERDICT` from the **current** validator set, which means a threshold of validators signing `"VERDICT_FINALIZED epoch_id_T"` off-chain: simultaneously a break of the honest-majority assumption A2 *and* a slashable equivocation (§4.1), since a `VERDICT_FINALIZED` signature with no corresponding canonical verdict is detectable double-signing. So covert deanonymization requires compromising a committee threshold **and** a validator (consensus) threshold **and** committing slashable equivocation — strictly stronger than the prior single-committee assumption. As before, any protocol-meaningful use of a recovered `null_v` requires a publicly visible `null_v_decryption` transaction. The privacy assumption is bounded by the Byzantine majority assumption already in the threat model, and now also by the public, slashable accountability of the validator attestation.
...
ForwardCommit construction with the 2-of-2 `null_v = s₁ + s₂` split and per-epoch commit_T publication (committee ciphertexts of `s₁` plus the validator ciphertext `d_T` of `s₂`); standing validator-attestation key `VA_pub` with proactive Shamir re-share across validator rotations (§4.1); `VERDICT_FINALIZED` validator threshold attestation emitted at verdict finalization, with a distinct DST and double-signing slashing for off-chain forgery; Statement 5 extended circuit (split check + per-slot ForwardCommit + `d_T` + SMT path) in Plonky3; DECRYPTION_SMT initialization; verdict_commit and verdict_reveal transaction types; null_v_decryption transaction type (both signatures) and permissionless aggregation; watchdog_signal transaction type; DKG protocol for per-epoch committee threshold BLS key.
...
ZK consistency proofs (Statements 1–3, 5); sign preservation constraint enforcement; ZK continuity proofs (arbitration committee); handoff ZK proof including PSI acknowledgment witnesses; rewind signal aggregation; desktop ZK benchmarks for all components (PoC gate); mobile benchmarks for all ZK components (data collection only — not a PoC gate, per §9.1 Mobile policy); Poseidon PRF security analysis (closed — OQ-1); domain separator collision check (closed — OQ-4); ForwardCommit security analysis (closed — OQ-2); extended Statement 5 mobile benchmark; dec_nullifier collision resistance analysis (closed — OQ-5); BLS-based ForwardCommit instantiation formal review; permutation reconstruction security benchmark; Pinkas PSI performance on mobile.
...
| OQ-15 | Does trust_total converge to a stable distribution under sustained adversarial flooding, or does it oscillate?                                                                                                                                                                              | Sybil resistance | theoretical              | 1    | 3      | **structurally resolved** — trust_total has no autonomous oscillation mode: contributions are non-negative and saturate at cap c; the novelty/item_weight damping is monotone-decreasing (negative feedback only); and §6.1 reputation has no trust_total term (no cross-coupling). Converges to a fixed point for fixed inputs. The only cyclic driver is the exogenous on-off reputation loop = §8.2 T1, calibrated in exp 5.4. See §7.3 and OQ-C1 (§10.2). |
...
| OQ-58 | What base Poisson rate λ for Loopix loop and drop cover traffic provides sufficient anonymity against a mix-node-observing adversary without imposing unacceptable bandwidth overhead on metered or mobile deployments, and what is the correct λ per config (A–E)?                         | Network          | empirical                | 3    | 2      | **analytical pre-bound available (keystone)** — a conservative ε_loopix(λ) follows from inverting the Loopix sender-anonymity bound (Piotrowska et al., CCS 2017) as a function of λ, mix topology (count/stratification), and adversary observation fraction; no experiment needed to obtain it (see OQ-58 note above). Unblocks numerical evaluation of OQ-19/46/55. Phase 5 exp 5.51 then refines λ per config A–E rather than deriving the bound from scratch. See §10.1.1. |
...
- **OQ-58 — Loopix λ → ε_loopix.** *Keystone.* Numerically blocks OQ-19, OQ-46, and OQ-55. A conservative `ε_loopix(λ)` is derivable analytically from Piotrowska et al. (CCS 2017) *before* any experiment (see the OQ-58 note above) — sequence this first; it is the highest-leverage item in the table.
...
// AUDITOR HANDOFF
{
  ...
  "commit_T":                 "<( c_T^{(0..N_fallback−1)} = ForwardCommit(s₁,'SUSPEND epoch_id_T',threshold_BLS_pk_T^{(i)};r),  d_T = ForwardCommit(s₂,'VERDICT_FINALIZED epoch_id_T',VA_pub;r) ),  s₁+s₂=null_v>",
  "threshold_bls_pk_T":       "<public key of this epoch's committee>",
  "successor_zk_proof":       "<proof: valid successor + leaf count consistency + Statement 5 (4 checks) + PSI acknowledgment witnesses>",
...
// ON-CHAIN EPOCH TRANSACTION
{
  ...
  "epoch_id":          "<node epoch ID>",
  "commit_T":          "<( committee ciphertexts of s₁, validator ciphertext d_T of s₂ ) — every epoch, exempt from n_commit>",
...
// ON-CHAIN NULL_V DECRYPTION
{
  ...
  "dec_nullifier":   "<Poseidon(verdict_hash, null_v)>",
  "sigma_suspend":   "<aggregated committee threshold BLS signature on 'SUSPEND epoch_id_T'>",
  "sigma_verdict":   "<validator threshold signature σ_T^VERDICT on 'VERDICT_FINALIZED epoch_id_T'>",
  "proof":           "<Verify(threshold_BLS_pk_T, 'SUSPEND epoch_id_T', sigma_suspend) = true
                       AND Verify(VA_pub, 'VERDICT_FINALIZED epoch_id_T', sigma_verdict) = true
                       AND ForwardCommit.Verify(c_T^{(i)}, s₁, sigma_suspend) = true
                       AND ForwardCommit.Verify(d_T, s₂, sigma_verdict) = true
                       AND s₁ + s₂ = null_v = true>"
}
