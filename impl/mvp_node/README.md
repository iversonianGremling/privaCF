...
| Class-2 first-observation audit + burst-score detection (real, `audit.rs`) → autonomous observer gossip | **VRF-selected observers** file ed25519-signed `FirstObservation` reports (only a node whose VRF lottery falls below θ may report — rate-limited, unspoofable); the detector takes the median first-seen epoch per subject and a **burst score** = subjects co-admitted within a window, flagging **admission-time Sybil cohorts** (the structural signal Phase-2 detection consumes). Remaining: wiring observer selection + report gossip into the live loop | SPEC §4.9.7, §7 |
...
| Pedersen preference commitment + behavioral Merkle history (real, `pedersen.rs`/`history.rs`) → wired into the epoch tx | **`C_p(T) = Σ pᵢ·Gᵢ + r·H`** vector Pedersen over Ristretto (hiding+binding, additively homomorphic so `C(T)−C(T−1)` commits to the difference — the Statement-3 / Layer-5 hook); **behavioral history** `M_v(T)` = salted Merkle leaves (announce/pull/audit/rate-limit) with per-epoch PRF salts → partial reveals keep siblings opaque. Standalone primitives; wiring `C_p`/`M_v` into the epoch transaction lands with Layer-5 / Statements 1–3 | SPEC §4.4, §4.6 |
| `Discovery` — bootstrap + chain address book + **real PSI primitive** (`psi.rs`) → networked PSI handshake | peers come from the genesis bootstrap + a dynamic address book; **private set intersection** is now a real **DH-PSI over Ristretto** — two nodes learn only the *size* of their shared-interest overlap (`should_connect` gates on θ), item sets stay hidden (pseudorandom blinded points). Remaining: the networked handshake over the mixnet (gossip referral → PSI exchange → connect); the deployment PSI is Pinkas-OT at scale | SPEC §5.3/§5.4 |
...
| Statement-5 ZK proof in the loop — **core GREEN, bridge AMBER** (`../spike_stmt5_proving/`, `../spike_modmul_cost/`) | the publish-`s₁` core (Poseidon `null_v`/`epoch_id` + SMT non-membership + `s₁+s₂`) is measured **GREEN (~30 ms)**; the VerEnc **bridge** (non-native BLS12-381 in-circuit) is now measured with a real full modular multiply (~512 rows/modmul) → a G1 scalar-mult ~2²¹ rows ~121 s naive = **RED**, **AMBER** with a 10–30× purpose-built gadget. So the in-loop proof is a tracked **Phase-1b** deliverable contingent on that gadget — NOT mandated per-epoch yet; until then `d_T` integrity is validator-side (the P1.2b sigma+range proof) | SPEC §4.9.5 |
