...
| SMT roots — real Poseidon SMT (`smt.rs`) → folded from verdicts | block headers carry **real circuit-matching Poseidon SMT roots** (SUSP_SMT + DECRYPTION_SMT), derived from the finalized chain and validated; membership/non-membership proofs match what Statement-5 verifies. Trees are empty until the verdict machinery (P1.5) folds suspensions/extractions | SPEC §4.9.2 |
