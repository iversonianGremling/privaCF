...
/// The §6.4 composite handoff proof: Statements 1 (norm), 2 (directional) and 3 (temporal) over a node's
/// two consecutive on-chain commitments `C_p(T−1)` and `C_p(T)`, proving the handed-off profile is
/// well-formed (bounded, sign-consistent, slow-moving) without revealing it. Generated by the profile
/// owner (only it knows the vector and per-epoch blindings); verifiable by anyone from the on-chain `c_p`.
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct HandoffStatements {
    pub norm: NormProof,
    pub temporal: TemporalProof,
    pub directional: DirectionalProof,
}

/// A departing node's on-chain §6.4 handoff-package proof: the composite statements bound to the node's
/// two consecutive on-chain commitments, identified by their `epoch_id`s so any validator looks the
/// commitments up from the chain and verifies. Recorded in `BlockHeader::handoff_proofs`.
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct HandoffProofRecord {
    pub subject: u64,      // epoch_id of C_p(T) — the profile being handed off
    pub prev_subject: u64, // epoch_id of C_p(T−1) — the previous commitment, for the temporal/directional binding
    pub stmts: HandoffStatements,
}

fn subseed(seed: &[u8; 32], tag: &[u8]) -> [u8; 32] {
    let mut h = blake3::Hasher::new();
    h.update(b"privacf-zk-handoff-subseed-v1");
    h.update(seed);
    h.update(tag);
    *h.finalize().as_bytes()
}

/// Prove the composite handoff statements for clean vectors `prev`/`curr` (with blindings `r_prev`/
/// `r_curr`): `‖curr‖∞ ≤ m`, `‖Δp‖∞ ≤ delta`, and the per-item `dirs` sign-consistency. The norm binds to
/// `C_p(T)`; the temporal/directional bind to `C_p(T) − C_p(T−1)`.
#[allow(clippy::too_many_arguments)]
pub fn prove_handoff(
    pc: &Pedersen,
    prev: &[i64],
    r_prev: &[u8; 32],
    curr: &[i64],
    ...
    HandoffStatements {
        norm: prove_norm(pc, curr, r_curr, m, &subseed(seed, b"norm")),
        temporal: prove_temporal(pc, prev, r_prev, curr, r_curr, delta, &subseed(seed, b"temporal")),
        directional: prove_directional(pc, prev, r_prev, curr, r_curr, dirs, m, &subseed(seed, b"directional")),
    }
}

/// Verify a composite handoff proof against the two on-chain commitments, enforcing the verifier's
/// protocol bounds: the proof's claimed norm bound must be `≤ m_max`, its temporal bound `≤ delta_max`,
/// its directional range bound `≤ m_max`, and all three sigma proofs must bind to the supplied `c_prev`/
/// `c_curr`. The bound checks stop a prover from claiming a vacuously large bound.
pub fn verify_handoff(
    ...
    s.norm.m <= m_max
        && s.temporal.delta <= delta_max
        && s.directional.bound <= m_max
        && verify_norm(pc, c_curr, &s.norm)
        && verify_temporal(pc, c_prev, c_curr, &s.temporal)
        && verify_directional(pc, c_prev, c_curr, &s.directional)
}
